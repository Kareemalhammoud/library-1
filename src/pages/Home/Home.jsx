...
const QUICK_ACTIONS = [
  {
    ...
    iconWrap: 'bg-[rgba(196,112,95,0.15)] border-[rgba(196,112,95,0.22)] text-[#00AB8E]',
    eyebrowColor: 'text-[#00AB8E]',
    detailColor: 'text-[#00AB8E]/80',
    icon: (
      <>
  ...
  },
  {
    ...
    href: '/events',
    colors: 'bg-[linear-gradient(150deg,rgba(246,249,255,1)_0%,rgba(236,243,253,1)_100%)] text-[#1C2B24] border-[#d8e5f6] dark:bg-[#1E293B] dark:border-[#334155] dark:text-[#F1F5F9]',
    iconWrap: 'bg-[rgba(74,110,163,0.09)] border-[rgba(74,110,163,0.14)] text-[#4A6EA3] dark:bg-[#243447] dark:border-[#3d5781] dark:text-[#93b7ee]',
    eyebrowColor: 'text-[#4A6EA3] dark:text-[#93b7ee]',
    detailColor: 'text-[#4A6EA3]/80 dark:text-[#93b7ee]/80',
    icon: (
      <>
  ...
  },
  {
    ...
    colors: 'bg-[linear-gradient(150deg,rgba(245,249,246,1)_0%,rgba(237,245,240,1)_100%)] text-[#1C2B24] border-[#d0ddd8] dark:bg-[#1E293B] dark:border-[#334155] dark:text-[#F1F5F9]',
    iconWrap: 'bg-[rgba(68,124,96,0.09)] border-[rgba(68,124,96,0.14)] text-[#44785E] dark:bg-[#1a3428] dark:border-[#2e5541] dark:text-[#8fd0af]',
    eyebrowColor: 'text-[#44785E] dark:text-[#8fd0af]',
    detailColor: 'text-[#44785E]/80 dark:text-[#8fd0af]/80',
    icon: (
      <>
  ...
  },
  {
    ...
    href: '/visit',
    colors: 'bg-[linear-gradient(150deg,rgba(255,252,243,1)_0%,rgba(250,243,226,1)_100%)] text-[#1C2B24] border-[#ead8a8] dark:bg-[#1E293B] dark:border-[#334155] dark:text-[#F1F5F9]',
...
function Home() {
  ...
  return (
    <div className="w-full bg-[#F2F5F3] text-[#1C2B24] dark:bg-[#0F172A] dark:text-[#F1F5F9]">
      <main>
        <section className="relative left-1/2 ml-[-50vw] h-[520px] w-screen overflow-hidden sm:h-[540px]" aria-label="Hero section with rotating images of the library and a search form">
            ...
            return (
              ...
              >
                <div className="mb-3 flex items-center gap-2">
                  <svg className="h-[13px] w-[13px] text-[#00AB8E]" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                    <rect x="3" y="5" width="18" height="16" rx="2" stroke="currentColor" strokeWidth="1.5" />
                    <path d="M3 10h18" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
                    <path d="M8 3v4M16 3v4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
                  </svg>
                  <span className="text-[0.58rem] font-bold uppercase tracking-[0.14em] text-[#00AB8E]">Upcoming Event</span>
                </div>
          ...
          <div className="absolute bottom-8 left-1/2 z-10 flex w-[calc(100%-2rem)] max-w-[370px] -translate-x-1/2 flex-col rounded-2xl border border-[rgba(0,103,81,0.28)] border-t-white/70 bg-[linear-gradient(160deg,rgba(240,248,244,0.96)_0%,rgba(225,240,235,0.92)_100%)] p-6 shadow-[0_1px_0_rgba(255,255,255,0.65)_inset,0_8px_32px_rgba(10,20,15,0.20),0_2px_8px_rgba(10,20,15,0.12),0_40px_80px_rgba(5,10,8,0.18)] backdrop-blur-[12px] sm:left-[12%] sm:top-1/2 sm:bottom-auto sm:w-[min(370px,88%)] sm:-translate-x-0 sm:-translate-y-1/2 sm:px-9 sm:py-8 dark:border-[#334155] dark:bg-[linear-gradient(160deg,rgba(15,23,42,0.92)_0%,rgba(30,41,59,0.88)_100%)]">
            ...
            <p className="mb-6 text-[0.875rem] leading-[1.7] text-[#4e4e4e] dark:text-[#94A3B8]">Spend time with a great book, join events, and share ideas with your community.</p>
            <form role="search" className="relative mb-4">
              <svg className="pointer-events-none absolute left-3 top-1/2 h-[15px] w-[15px] -translate-y-1/2 text-[#006751]/50 dark:text-[#2DD4A8]/70" viewBox="0 0 24 24" fill="none" aria-hidden="true">
              ...
              <input className="w-full rounded-lg border border-[rgba(0,103,81,0.20)] bg-white/80 px-10 py-3 text-[0.8rem] text-[#1C2B24] outline-none transition focus:border-[rgba(0,103,81,0.55)] focus:bg-white focus:ring-4 focus:ring-[#006751]/10 dark:border-[#334155] dark:bg-[#0F172A]/80 dark:text-[#F1F5F9] dark:focus:border-[#2DD4A8] dark:focus:ring-[#2DD4A8]/10" type="search" placeholder="Search books, authors, or subjects..." aria-label="Search the library catalog" />
            </form>
            <div className="flex flex-col gap-2 sm:flex-row">
              <button type="button" className="rounded-md bg-[#006751] px-5 py-2.5 text-[0.8rem] font-semibold text-white shadow-[0_1px_3px_rgba(0,103,81,0.3)] transition hover:bg-[#005040] dark:bg-[#2DD4A8] dark:text-[#0F172A] dark:hover:bg-[#14B88A]" onClick={() => navigate('/register')}>Get Started</button>
              <button type="button" className="rounded-md border border-[rgba(28,43,36,0.22)] px-5 py-2.5 text-[0.8rem] font-semibold text-[#1C2B24] transition hover:bg-[rgba(28,43,36,0.05)] dark:border-[#334155] dark:text-[#F1F5F9] dark:hover:bg-[#1E293B]">Learn More</button>
            </div>
          </div>
        </section>

        <section className="grid grid-cols-2 border-y border-[rgba(0,103,81,0.12)] bg-[rgba(0,103,81,0.04)] py-7 text-center sm:grid-cols-4 dark:border-[#334155] dark:bg-[rgba(45,212,168,0.04)]" aria-label="Library statistics">
          {[
            ...
            <div key={label} className="flex flex-col items-center gap-1 border-r border-[rgba(0,103,81,0.18)] px-4 last:border-r-0 dark:border-[#334155]">
              <span className="text-[1.3rem] font-bold tracking-[-0.03em] text-[#1C2B24] dark:text-[#F1F5F9]">{value}</span>
              <span className="text-[0.62rem] uppercase tracking-[0.08em] text-[rgba(28,43,36,0.38)] dark:text-[#94A3B8]">{label}</span>
            </div>
          ))}
        </section>
        ...
        <section className="relative px-4 py-16 sm:px-6 lg:px-8 lg:py-24">
          <div className="mx-auto max-w-[var(--container-max)]">
            <div className="mb-12">
              <h2 className="mb-3 text-[clamp(1.45rem,2.8vw,1.9rem)] font-extrabold tracking-[-0.032em] text-[#1C2B24] before:mb-[1.15rem] before:block before:h-[3px] before:w-10 before:rounded-full before:bg-gradient-to-r before:from-[#006751] before:to-[rgba(0,103,81,0.35)] dark:text-[#F1F5F9] dark:before:from-[#2DD4A8] dark:before:to-[rgba(45,212,168,0.25)]">Explore the Library</h2>
              <p className="max-w-[52ch] text-[0.9rem] leading-[1.72] text-[#595959] dark:text-[#94A3B8]">Browse collections, join events, find study spaces, and plan your visit.</p>
            </div>
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              {QUICK_ACTIONS.map((card) => (
                ...
                >
                  ...
                  <p className={`text-[0.8rem] leading-[1.72] ${card.title === 'Find Your Next Book' ? 'text-white/65' : 'text-[#5a5a5a] dark:text-[#94A3B8]'}`}>{card.body}</p>
        ...
        <section className="relative left-1/2 ml-[-50vw] w-screen overflow-hidden border-y border-[rgba(0,103,81,0.22)] bg-[linear-gradient(180deg,rgba(215,235,228,0.38)_0%,rgba(200,228,220,0.54)_100%)] px-4 py-8 sm:px-6 lg:px-8 dark:border-[#334155] dark:bg-[linear-gradient(180deg,rgba(15,23,42,0.95)_0%,rgba(30,41,59,0.92)_100%)]">
          <div className="mx-auto max-w-[var(--container-max)]">
            <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
              <div>
                <h2 className="mb-2 text-[clamp(1.1rem,2vw,1.35rem)] font-bold tracking-[-0.022em] text-[#1C2B24] before:mb-3 before:block before:h-[2px] before:w-8 before:rounded-full before:bg-gradient-to-r before:from-[#006751] before:to-[rgba(0,103,81,0.35)] dark:text-[#F1F5F9] dark:before:from-[#2DD4A8] dark:before:to-[rgba(45,212,168,0.25)]">Staff Picks</h2>
                <p className="max-w-[52ch] text-[0.82rem] leading-[1.65] text-[#6a6a6a] dark:text-[#94A3B8]">Discover a selection of titles to inspire your next visit.</p>
              </div>
              <a href="/books" className="text-[0.775rem] font-semibold text-[#006751] transition hover:opacity-100 dark:text-[#2DD4A8]" onClick={(e) => { e.preventDefault(); navigate('/books') }}>View All</a>
            </div>
                ...
                {TRACK.map((book, i) => (
                  <article key={`${book.id}-${i}`} className="group shrink-0 cursor-pointer" onClick={() => navigate(`/books/${book.id}`)} aria-label={`View details for ${book.title} by ${book.author}`}>
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      <h3 className="text-[0.78rem] font-bold leading-[1.25] tracking-[-0.015em] text-[#1C2B24] dark:text-[#F1F5F9]">{book.title}</h3>
                      <p className="text-[0.66rem] italic text-[#8a8a8a] dark:text-[#94A3B8]">{book.author}</p>
                    </div>
                  </article>
                ))}
              </div>
