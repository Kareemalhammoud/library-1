...
export default function ListView() {
  ...
  // Controls whether the filter panel is expanded on mobile/tablet
    ...
    <div className="min-h-screen bg-[#f8f7f4] pb-20 dark:bg-[#0f0f0f]">
      ...
      >
        <div className="mx-auto max-w-[1200px]">
          ...
          {/* Row 1: search input + mobile filter toggle button + sort dropdown */}
          <div className="flex flex-wrap items-end gap-2 sm:gap-3">
            ...
            {/* "Filters" toggle — only shown on mobile/tablet (hidden md+) */}
            <button
          ...
          {/* Row 2: filter dropdowns
              - Mobile/tablet: hidden by default, revealed by the Filters button above, stacked 2-per-row
              - Desktop (md+): always visible in a single horizontal row */}
          <div
            id="filter-panel"
        ...
        {filtered.length === 0 ? (
          ...
          <>
            {loading ? (
              ...
              /* Books: 2 cols on mobile → 3 on tablet → 4 on desktop → 5 on wide */
              <ul
                className="m-0 grid list-none grid-cols-2 gap-x-4 gap-y-6 p-0 sm:grid-cols-3 md:grid-cols-4 lg:gap-x-6 lg:gap-y-8 xl:grid-cols-5"
